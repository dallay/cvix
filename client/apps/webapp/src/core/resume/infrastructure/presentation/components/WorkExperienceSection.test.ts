...
describe('WorkExperienceSection.vue', () => {
  ...
  describe('v-model binding', () => {
    ...
    it('should bind position field', async () => {
      ...
      expect((positionInput.element as HTMLInputElement).value).toBe('Tech Lead');
    });
    ...
    it('should bind start and end dates', async () => {
      ...
      expect((startDateInput.element as HTMLInputElement).value).toBe('2020-01-01');
      expect((endDateInput.element as HTMLInputElement).value).toBe('2022-12-31');
    });
  ...
  describe('highlights management', () => {
    it('should render highlights when provided', () => {
      ...
      expect((highlight1.element as HTMLInputElement).value).toBe('Improved performance by 50%');
      expect((highlight2.element as HTMLInputElement).value).toBe('Led team of 5');
    });
    ...
    it('should handle empty highlights array', () => {
      ...
      const wrapper = mountComponent(work);
      expect(wrapper.find('[data-testid="work-highlight-0-0"]').exists()).toBe(false);
    });
  });
  ...
  describe('form validation', () => {
    it('should have form fields for work entry', () => {
      ...
      expect(wrapper.find('[data-testid="work-name-0"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="work-position-0"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="work-start-date-0"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="work-end-date-0"]').exists()).toBe(true);
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle multiple work entries with highlights', () => {
      ...
      expect(wrapper.find('[data-testid="work-highlight-0-0"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="work-highlight-1-0"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="work-highlight-1-1"]').exists()).toBe(true);
    });
  });
});
