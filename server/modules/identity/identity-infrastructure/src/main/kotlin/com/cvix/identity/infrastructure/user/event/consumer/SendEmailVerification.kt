package com.cvix.identity.infrastructure.user.event.consumer

import com.cvix.common.domain.bus.event.EventConsumer
import com.cvix.common.domain.bus.event.Subscribe
import com.cvix.identity.domain.user.event.UserCreatedEvent
import com.cvix.identity.infrastructure.user.persistence.keycloak.KeycloakRepository
...
class SendEmailVerification(private val keycloakRepository: KeycloakRepository) :
    ...
    override suspend fun consume(event: UserCreatedEvent) {
        ...
        runCatching {
            keycloakRepository.verify(event.id.toString())
        }.onFailure { ex ->
