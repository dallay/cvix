...
describe("BasicsSection.vue", () => {
	...
	describe("rendering", () => {
		it("should render all form fields", () => {
			...
			expect(wrapper.find('[data-testid="fullname-input"]').exists()).toBe(
				true,
			);
			expect(
				wrapper.find('[data-testid="label-short-description-input"]').exists(),
			).toBe(true);
			expect(
				wrapper.find('[data-testid="profile-picture-input"]').exists(),
			).toBe(true);
	...
	describe("form validation attributes", () => {
		...
		it("should have correct input types", () => {
			...
			expect(
				wrapper.find('[data-testid="email-input"]').attributes("type"),
			).toBe("email");
			expect(
				wrapper.find('[data-testid="phone-input"]').attributes("type"),
			).toBe("tel");
			expect(wrapper.find('[data-testid="url-input"]').attributes("type")).toBe(
				"url",
			);
		});
		...
		it("should have maxlength on name and label fields", () => {
			...
			expect(
				wrapper.find('[data-testid="fullname-input"]').attributes("maxlength"),
			).toBe("100");
			expect(
				wrapper
					.find('[data-testid="label-short-description-input"]')
	...
	describe("user interactions", () => {
		it("should update multiple fields", async () => {
			...
			await wrapper
				...
				.setValue("jane@test.com");
			await wrapper.find("#city").setValue("Boston");

			expect(
				(
					wrapper.find('[data-testid="fullname-input"]')
	...
	describe("edge cases", () => {
		...
		it("should handle long text input", async () => {
			...
			expect((summaryTextarea.element as HTMLTextAreaElement).value).toBe(
				longText,
			);
		});
