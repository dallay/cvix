...
describe('ReferenceSection.vue', () => {
  ...
  describe('rendering', () => {
    ...
    it('should render reference entry when provided', () => {
      ...
      expect(wrapper.find('[data-testid="reference-name-0"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="reference-text-0"]').exists()).toBe(true);
    });
    ...
    it('should render multiple reference entries', () => {
      ...
      const wrapper = mountComponent(references);
      expect(wrapper.find('[data-testid="reference-name-0"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="reference-name-1"]').exists()).toBe(true);
    });
  ...
  describe('v-model binding', () => {
    ...
    it('should bind reference text field', async () => {
      ...
      expect((textArea.element as HTMLTextAreaElement).value).toBe('Excellent professional');
    });
    ...
    it('should display pre-filled data', () => {
      const references: Reference[] = [
        {
          name: 'Jane Doe',
          reference: 'Jane was an outstanding colleague who consistently delivered high-quality work.',
        },
      ];
      ...
      expect((nameInput.element as HTMLInputElement).value).toBe('Jane Doe');
      expect((textArea.element as HTMLTextAreaElement).value).toContain('outstanding colleague');
    });
  });
  ...
  describe('form validation', () => {
    it('should mark required fields', () => {
      ...
      const wrapper = mountComponent(references);
      expect(wrapper.find('[data-testid="reference-name-0"]').attributes('required')).toBeDefined();
      expect(wrapper.find('[data-testid="reference-text-0"]').attributes('required')).toBeDefined();
    });
    ...
    it('should have correct input types', () => {
      ...
      const wrapper = mountComponent(references);
      expect(wrapper.find('[data-testid="reference-name-0"]').attributes('type')).toBe('text');
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle single reference', () => {
      ...
      const wrapper = mountComponent(references);
      expect(wrapper.find('[data-testid="reference-name-0"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="reference-name-1"]').exists()).toBe(false);
    });
    ...
    it('should handle multiple references with varying text lengths', () => {
      const references: Reference[] = [
        ...
        {
          name: 'John Smith',
          reference: 'This is a much longer reference that contains multiple sentences and provides detailed information about the candidate\'s performance and character.',
        },
      ];
      ...
      expect((ref1.element as HTMLTextAreaElement).value).toBe('Short reference');
      expect((ref2.element as HTMLTextAreaElement).value.length).toBeGreaterThan(50);
    });
