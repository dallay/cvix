...
describe("CertificateSection.vue", () => {
	...
	describe("rendering", () => {
		...
		it("should render certificate entry when provided", () => {
			...
			expect(wrapper.find('[data-testid="certificate-name-0"]').exists()).toBe(
				true,
			);
		});
		...
		it("should render multiple certificate entries", () => {
			...
			expect(wrapper.find('[data-testid="certificate-name-1"]').exists()).toBe(
				true,
			);
		});
	...
	describe("v-model binding", () => {
		...
		it("should bind issuer field", async () => {
			...
			expect((issuerInput.element as HTMLInputElement).value).toBe(
				"Amazon Web Services",
			);
		});
	...
	describe("form validation", () => {
		it("should mark required fields", () => {
			...
			expect(
				wrapper
					.find('[data-testid="certificate-issuer-0"]')
					.attributes("required"),
			).toBeDefined();
		});
		...
		it("should have correct input types", () => {
			...
			expect(
				wrapper.find('[data-testid="certificate-url-0"]').attributes("type"),
			).toBe("url");
		});
	});
	...
	describe("edge cases", () => {
		...
		it("should handle multiple certificates from different issuers", () => {
			...
			expect(wrapper.find('[data-testid="certificate-name-2"]').exists()).toBe(
				true,
			);
		});
		...
		it("should handle long certificate names", () => {
			...
			const nameInput = wrapper.find('[data-testid="certificate-name-0"]');
			expect(
				(nameInput.element as HTMLInputElement).value.length,
			).toBeGreaterThan(50);
		});
	});
});
