...
describe('AwardSection.vue', () => {
  ...
  describe('v-model binding', () => {
    it('should bind title field', async () => {
      ...
      expect((titleInput.element as HTMLInputElement).value).toBe('Employee of the Year');
    });
    ...
    it('should bind awarder field', async () => {
      ...
      expect((awarderInput.element as HTMLInputElement).value).toBe('Tech Company Inc.');
    });
    ...
    it('should bind summary field', async () => {
      ...
      expect((summaryInput.element as HTMLTextAreaElement).value).toBe('Awarded for outstanding performance');
    });
    ...
    it('should display pre-filled data', () => {
      ...
      expect((titleInput.element as HTMLInputElement).value).toBe('Best Developer');
      expect((dateInput.element as HTMLInputElement).value).toBe('2021-06-15');
      expect((awarderInput.element as HTMLInputElement).value).toBe('Tech Corp');
    });
  });
  ...
  describe('form validation', () => {
    it('should mark required fields', () => {
      ...
      const wrapper = mountComponent(awards);
      expect(wrapper.find('[data-testid="award-title-0"]').attributes('required')).toBeDefined();
      expect(wrapper.find('[data-testid="award-date-0"]').attributes('required')).toBeDefined();
      expect(wrapper.find('[data-testid="award-awarder-0"]').attributes('required')).toBeDefined();
    });
    ...
    it('should have correct input types', () => {
      ...
      const wrapper = mountComponent(awards);
      expect(wrapper.find('[data-testid="award-title-0"]').attributes('type')).toBe('text');
      expect(wrapper.find('[data-testid="award-date-0"]').attributes('type')).toBe('date');
      expect(wrapper.find('[data-testid="award-awarder-0"]').attributes('type')).toBe('text');
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle single award', () => {
      ...
      const wrapper = mountComponent(awards);
      expect(wrapper.find('[data-testid="award-title-0"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="award-title-1"]').exists()).toBe(false);
    });
      ...
      const awards: Award[] = [
        {
          ...
          summary: 'This award recognizes an individual who has made significant and lasting contributions to the field over many years. The recipient has demonstrated exceptional dedication, innovation, and leadership throughout their career.',
        },
      ];

      const wrapper = mountComponent(awards);
      const summaryInput = wrapper.find('[data-testid="award-summary-0"]');
      expect((summaryInput.element as HTMLTextAreaElement).value.length).toBeGreaterThan(100);
    });
