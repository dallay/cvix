...
The authoritative registry for the CVIX TexLive Docker image is `ghcr.io/cvix/texlive`. Users should pull the official image from this registry to ensure they are using the latest and verified version. Local builds (`cvix/texlive`) are intended for development and testing purposes only.

### Examples

#### Pulling the Official Image
```bash
docker pull ghcr.io/cvix/texlive:latest
```
...
For production use, always pull from `ghcr.io/cvix/texlive` as it includes multi-architecture support. The following command is used to build and push multi-arch images:
```bash
docker buildx build \
  --platform linux/amd64,linux/arm64 \
  -t ghcr.io/cvix/texlive:latest \
