...
- [ ] T028a [US3] Provision DLQ infrastructure using IaC (Terraform/CloudFormation/CDK) for a DynamoDB table with columns (id, payload, error_reason, timestamp, retry_count) and TTL for retention. Create the IaC script at `infra/dlq-provision.tf` or CloudFormation/CDK equivalent (do NOT use SQL/psql for DynamoDB). Define IAM roles for access control, set a 30-day retention policy, and configure monitoring with alerts for retry_count thresholds. Provide an example AWS CLI snippet or Terraform template for operators.
