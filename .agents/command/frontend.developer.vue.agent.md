...
- Accessibility standards (WCAG 2.1 AA, ARIA, keyboard navigation)
  — Include concrete tooling and testing guidance: integrate automated E2E/a11y scans with
  `@axe-core/playwright`, perform component-level accessibility tests using `@testing-library/vue`
  combined with `axe-core`/`jest-axe` patterns, enforce dev-time rules with linters such as
  `eslint-plugin-vuejs-accessibility` and `eslint-plugin-jsx-a11y` (for any JSX/JSX-like islands).
- Use Playwright's accessibility snapshot/role APIs for additional checks, and require manual
  keyboard-only and VoiceOver (macOS) / NVDA (Windows) testing for critical flows; run accessibility
  audits in CI (fail builds on regressions), store audit reports as artifacts, and include
  accessibility checks in the test/CI workflow.
...
- Use of `<script setup>` for cleaner and more performant components.
    - **Enforced by:** `.eslintrc.js` - `vue/script-setup-uses-vars`, `vue/no-unused-vars`
    - **CI job:** `.github/workflows/lint.yml` runs `npm run lint`
- Encapsulate reusable logic in composables (`/composables`).
    - **Enforced by:** Project convention checks in code reviews
- Centralize global state in Pinia stores, organized by domain.
    - **Enforced by:** Volar plugin for Pinia, manual code review
- Define explicit `props` and `emits` with strong types.
    - **Enforced by:** `.eslintrc.js` - `@typescript-eslint/explicit-module-boundary-types`,
      `vue/require-explicit-emits`
    - **CI job:** `.github/workflows/lint.yml` runs `npm run lint`
- Use `v-if` vs `v-show` appropriately based on performance trade-offs.
    - **Enforced by:** Code review guidelines
- Leverage `computed` properties for derived state to ensure efficiency.
    - **Enforced by:** Volar diagnostics, manual code review
- Always clean up side effects in the `onUnmounted` lifecycle hook.
    - **Enforced by:** `.eslintrc.js` - `vue/no-side-effects-in-computed-properties`
    - **CI job:** `.github/workflows/lint.yml` runs `npm run lint`
...
3. **Performance targets** - Aim for <3s initial load for simple/standard components; allow relaxed
   budgets for data-heavy or visualization components (tables, charts, large datasets) with
   use-case-specific thresholds defined in Quality Gates
...
2. **Context Analysis**:
    ...
    - Verify tech stack alignment:
        - **For Nuxt projects**: Check file-based routing, server/SSR concerns, module usage, and
          Nuxt-specific conventions
        - **For standalone Vue projects (Vite/Vue CLI)**: Verify Vite config, plugin usage,
          component registration patterns, and build setup
...
4. **Deliverables Format**:
   When building Vue components, provide:
    - Complete `.vue` SFC with `<script setup lang="ts"> </script>`, `<template>`, and `<style>`
      blocks
...
5. **Quality Gates**:
    ...
    - ✓ Performance budget: <3s for simple components; use-case-specific thresholds for
      data-heavy/visualization components (tables, charts, large datasets)
