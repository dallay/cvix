...
describe("ProfilesField.vue", () => {
	...
	describe("rendering", () => {
		...
		it("should render profile entry when provided", () => {
			...
			expect(wrapper.find('[data-testid="profile-network-0"]').exists()).toBe(
				true,
			);
		});
		...
		it("should render multiple profile entries", () => {
			...
			expect(wrapper.find('[data-testid="profile-network-1"]').exists()).toBe(
				true,
			);
		});
	...
	describe("form validation", () => {
		it("should have correct input types", () => {
			...
			const wrapper = mountComponent(profiles);
			expect(
				wrapper.find('[data-testid="profile-network-0"]').attributes("type"),
			).toBe("text");
			expect(
				wrapper.find('[data-testid="profile-username-0"]').attributes("type"),
			).toBe("text");
			expect(
				wrapper.find('[data-testid="profile-url-0"]').attributes("type"),
			).toBe("url");
		});
	});
	...
	describe("edge cases", () => {
		...
		it("should handle multiple social networks", () => {
			...
			expect(wrapper.find('[data-testid="profile-network-2"]').exists()).toBe(
				true,
			);
			expect(wrapper.find('[data-testid="profile-network-3"]').exists()).toBe(
				true,
			);
		});
		...
		it("should handle different URL formats", () => {
			...
			expect((url0.element as HTMLInputElement).value).toContain(
				"linkedin.com",
			);
		...
		it("should handle professional vs personal profiles", () => {
			...
			expect((username0.element as HTMLInputElement).value).toContain(
				"professional",
			);
			expect((username1.element as HTMLInputElement).value).toContain(
				"personal",
			);
		});
	});
