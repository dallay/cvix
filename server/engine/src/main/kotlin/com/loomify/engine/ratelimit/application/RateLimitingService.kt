...
class RateLimitingService(
    ...
    fun consumeToken(identifier: String, endpoint: String): Mono<RateLimitResult> {
        return consumeToken(identifier, endpoint, RateLimitStrategy.BUSINESS)
    }

    /**
     * Consumes a token for a given identifier using a specific strategy and publishes an event if the rate limit is exceeded.
     *
     * @param identifier The identifier to rate limit (e.g., API key or IP address).
     * @param endpoint The endpoint being accessed.
