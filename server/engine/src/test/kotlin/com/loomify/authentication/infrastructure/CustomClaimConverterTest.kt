...
import com.loomify.authentication.domain.Role
...
internal class CustomClaimConverterTest {
    ...
    private fun user(): UserBuilder = UserBuilder(this::mockUser)
    ...
    @Test
    fun shouldSkipUserLoadingOnReactiveThread() {
        // Create a thread with a reactive thread name to simulate reactor-http-nio-2
        val reactiveThreadName = "reactor-http-nio-2"
        val originalThread = Thread.currentThread()
        val originalName = originalThread.name

        try {
            // Temporarily change the thread name to simulate a reactive thread
            originalThread.name = reactiveThreadName

            // No need to mock WebClient since it should not be called on reactive threads
            val convertedClaims = converter.convert(simpleClaim())

            // Should only return the basic claims without attempting to load user info
            assertThat(convertedClaims).containsExactly(entry("sub", "123"))
        } finally {
            // Restore original thread name
            originalThread.name = originalName
        }
    }

    companion object {
        private fun simpleClaim(): Map<String, Any> = mapOf("sub" to "123")

        private fun anotherSimpleClaim(): Map<String, Any> = mapOf("sub" to "456")
    }
}
